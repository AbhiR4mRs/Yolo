...
class ChannelAttention(nn.Module):
    """
    This is the 'CA Block' from Figure 4.
    It implements the channel attention mechanism described in the paper.
    """
    def __init__(self, inp, oup, reduction=32):
        super(ChannelAttention, self).__init__()
        # As per Figure 4: X Avg Pool and Y Avg Pool
        self.pool_h = nn.AdaptiveAvgPool2d((None, 1))
        self.pool_w = nn.AdaptiveAvgPool2d((1, None))

        # As per Figure 4: Concat + Conv2d, BatchNorm + Non-linear
        mip = max(8, inp // reduction)
        self.conv1 = nn.Conv2d(inp, mip, 1, 1, 0)
        self.bn1 = nn.BatchNorm2d(mip)
        self.act = h_swish() # Paper uses h-sigmoid, but h-swish is common in MobileNetV3
                            # Let's stick closer to the diagram which shows "Non-linear"

        # As per Figure 4: Two separate Conv2d + h-sigmoid
    ...
    def forward(self, x):
        ...
        # 1. Avg Pooling (X Avg Pool and Y Avg Pool)
        x_h = self.pool_h(x)
        ...
        # 6. Apply attention (multiplication)
        # This matches Eq. 1: Y = X * g_h * g_w
        out = identity * a_w * a_h

        return out


# ... (Make sure the h_sigmoid, h_swish, and ChannelAttention classes are above this) ...

class TransformerEncoderLayer(nn.Module):
    """
    This is the 'Transformer Block' from Figure 5.
    It's a single layer of a Transformer encoder.
    """
    def __init__(self, c, num_heads):
        super().__init__()
        ...
        self.fc1 = nn.Linear(c, c * 4) # Standard expansion factor of 4
        self.fc2 = nn.Linear(c * 4, c)
        self.act = nn.GELU() # Using GELU as a standard transformer activation
        self.ln2 = nn.LayerNorm(c)

    def forward(self, x):
        # x shape is (N, B, C) where N=sequence length, B=batch size, C=channels
        # YOLOv5/v8 uses (B, C, H, W). We need to reshape.
        
        b, c, h, w = x.shape
        
        # 1. Flatten and permute for MHSA
        # (B, C, H, W) -> (B, C, N) -> (N, B, C)
        x_flat = x.flatten(2).permute(2, 0, 1) # N=H*W
        
        # 2. Apply MHSA
        # As per Figure 5: Input -> MHSA -> Add -> LayerNorm
        attn_output, _ = self.ma(x_flat, x_flat, x_flat)
        x_flat = self.ln1(x_flat + attn_output) # Add & Norm
        
        # 3. Apply MLP
        # As per Figure 5: -> MLP -> Add -> LayerNorm
        mlp_output = self.fc2(self.act(self.fc1(x_flat)))
        x_flat = self.ln2(x_flat + mlp_output) # Add & Norm
        
        # 4. Reshape back
        # (N, B, C) -> (B, C, N) -> (B, C, H, W)
        out = x_flat.permute(1, 2, 0).reshape(b, c, h, w)
        return out


class TransformerBlockPaper(nn.Module):
    """
    A block of N TransformerEncoderLayers.
    The paper adds *one* Transformer module, so we'll default to N=1.
    """
    def __init__(self, c1, c2, num_heads=4, num_layers=1):
        super().__init__()
        # This module just wraps N layers
        self.layers = nn.Sequential(*[TransformerEncoderLayer(c1, num_heads) for _ in range(num_layers)])

    def forward(self, x):
        return self.layers(x)
    

# ... (All your other classes from common.py) ...
# ... (h_sigmoid, h_swish, ChannelAttention, TransformerEncoderLayer, TransformerBlock) ...

class CA_Bneck(nn.Module):
    """
    This is the full 'CA-Bneck' module from Figure 4.
    It's a MobileNetV3 Bneck block that uses our ChannelAttention module.
    """
    def __init__(self, inp, oup, kernel_size, stride, exp_size, use_ca):
        super(CA_Bneck, self).__init__()
        assert stride in [1, 2]
        self.use_residual = (stride == 1 and inp == oup)
        
        # 1. Point-wise expansion (Conv2d 1x1)
        self.conv_expand = nn.Conv2d(inp, exp_size, 1, 1, 0, bias=False)
        self.bn_expand = nn.BatchNorm2d(exp_size)
        self.act = h_swish() # As used in MobileNetV3

        # 2. Depth-wise convolution
        self.conv_depthwise = nn.Conv2d(exp_size, exp_size, kernel_size, stride,
                                      padding=(kernel_size - 1) // 2, groups=exp_size, bias=False)
        self.bn_depthwise = nn.BatchNorm2d(exp_size)

        # 3. Channel Attention block
        # This is where we plug in the module we defined in Step 1
        self.ca = ChannelAttention(exp_size, exp_size) if use_ca else nn.Identity()

        # 4. Point-wise projection (Conv2d 1x1)
        self.conv_project = nn.Conv2d(exp_size, oup, 1, 1, 0, bias=False)
        self.bn_project = nn.BatchNorm2d(oup)

    def forward(self, x):
        identity = x

        # Follow the diagram in Figure 4
        out = self.act(self.bn_expand(self.conv_expand(x)))
        out = self.bn_depthwise(self.conv_depthwise(out))
        
        # This is the "X" (multiply) in Figure 4, implemented inside our CA module
        out = self.ca(out)
        
        out = self.bn_project(self.conv_project(out))
...
# ... (All your other classes from common.py) ...
# ... (h_sigmoid, h_swish, ChannelAttention, TransformerEncoderLayer, TransformerBlock, CA_Bneck) ...

class BiFPN_Lite(nn.Module):
    """
    This is the 'BiFPN-Lite Block' from Figure 6.
    It takes 3 inputs P3, P4, P5 (from 52x52, 26x26, 13x13 backbone layers).
    It outputs 3 fused feature maps N3, N4, N5 for the YOLO Head.
    """
    def __init__(self, c_p3, c_p4, c_p5):
        super(BiFPN_Lite, self).__init__()
        
        # This implementation is based directly on Figure 6
        # The paper shows Conv*5. We'll use a Conv block for this.
        # c_p3, c_p4, c_p5 are the input channels from the backbone
        # The diagram shows all internal channels are the same, let's assume c_p3 (e.g., 256)
        
        c_ = c_p3 # Use the smallest channel size as the unified channel size
        
        # --- Top-down path ---
        # P4_td: (P4_in) + Upsample(P5_td)
        self.p4_td_conv = Conv(c_p4 + c_, c_, 1, 1) # Concat + Conv*5
        self.upsample = nn.Upsample(scale_factor=2, mode='nearest')
        
        # P3_out: (P3_in) + Upsample(P4_td)
        self.p3_out_conv = Conv(c_p3 + c_, c_, 1, 1) # Concat + Conv*5

        # --- Bottom-up path ---
        # P4_out: (P4_td) + Downsample(P3_out)
        self.p4_out_conv = Conv(c_ + c_, c_, 1, 1) # Concat + Conv*5
        self.downsample_p4 = Conv(c_, c_, 3, 2)
        
        # P5_out: (P5_in) + Downsample(P4_out)
        self.p5_out_conv = Conv(c_p5 + c_, c_, 1, 1) # Concat + Conv*5
        self.downsample_p5 = Conv(c_, c_, 3, 2)
        
        # Input convs for P3, P4, P5 to unify channels
        self.p3_in_conv = Conv(c_p3, c_, 1, 1)
        self.p4_in_conv = Conv(c_p4, c_, 1, 1)
        self.p5_in_conv = Conv(c_p5, c_, 1, 1)


    def forward(self, inputs):
        # P3_in, P4_in, P5_in are the 3 feature maps from the backbone
        P3_in, P4_in, P5_in = inputs
        
        # Unify input channels
        P3_in = self.p3_in_conv(P3_in)
        P4_in = self.p4_in_conv(P4_in)
        P5_in = self.p5_in_conv(P5_in)

        # --- Top-down path (Figure 6, pink boxes) ---
        # P5_td is just P5_in
        P5_td = P5_in 
        
        # P4_td = P4_in + Upsample(P5_td)
        P4_td = self.p4_td_conv(torch.cat([P4_in, self.upsample(P5_td)], 1))
        
        # P3_out = P3_in + Upsample(P4_td)
        N3 = self.p3_out_conv(torch.cat([P3_in, self.upsample(P4_td)], 1))

        # --- Bottom-up path (Figure 6, blue/pink boxes) ---
        # P4_out = P4_td + Downsample(P3_out)
        N4 = self.p4_out_conv(torch.cat([P4_td, self.downsample_p4(N3)], 1))
        
        # P5_out = P5_td + Downsample(P4_out)
        N5 = self.p5_out_conv(torch.cat([P5_td, self.downsample_p5(N4)], 1))

        return N3, N4, N5 # The 3 outputs for YOLO Head